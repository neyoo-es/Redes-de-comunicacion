...
public class NFControllerLogicP2P {
	...
	protected boolean downloadFromPeers(NFControllerLogicDir dirLogic, String targetPeerNickname,
		...
		if (targetPeerNickname.equals("*")) {
		    // Usar todos los peers registrados en el directorio
		    serverAddresses = peers.values().toArray(new InetSocketAddress[0]);
		} else {
		    // Usar únicamente el peer indicado por nickname
	...
	protected boolean downloadFileFromServers(InetSocketAddress[] serverAddressList, String targetHashSubstring) {
		...
		java.nio.file.Path localPath =
		        es.um.redes.nanoFiles.util.FileNameUtil.chooseAvailableName(remoteFileName);

		// 3. Descarga alternando chunks entre los servidores disponibles
		try {
		    ...
		    // 5. Cerrar todos los sockets al terminar
