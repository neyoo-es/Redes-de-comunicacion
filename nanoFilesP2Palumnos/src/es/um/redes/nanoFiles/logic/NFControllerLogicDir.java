...
public class NFControllerLogicDir {
	...
	protected boolean downloadAndSaveFromDirectory(String hashSubstring) {
		...
		try {
			java.nio.file.Path sharedDir = java.nio.file.Paths.get(es.um.redes.nanoFiles.application.NanoFiles.sharedDirname);
			if (!java.nio.file.Files.exists(sharedDir)) {
				java.nio.file.Files.createDirectories(sharedDir);
			}
			java.nio.file.Path dest = es.um.redes.nanoFiles.util.FileNameUtil.chooseAvailableName(
					sharedDir.resolve(dl.filename).toString());
			...
			System.out.println("* Downloaded directory file to " + toDisplayPath(dest) + " (" + dl.data.length+ " bytes)");
			if (dl.filehash != null) {
