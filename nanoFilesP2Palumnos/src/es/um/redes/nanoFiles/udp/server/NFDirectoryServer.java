...
public class NFDirectoryServer {
	...
	private void sendResponseTestMode(DatagramPacket pkt) throws IOException {
		...
		//construimos el string a partir de pkt
		
		InetSocketAddress clientAddr = (InetSocketAddress) pkt.getSocketAddress();		//debemos sacar la dirección desde recibimos la info de pkt para devoler pingok
		System.out.println("Receiving datagram with message \"" + messageFromClient + "\"");
		System.out.println("Destination is client at addr: " + clientAddr);

		String testText = null;		//el objeto que sera pingok (el string de respuesta valgame)
		
		if (messageFromClient.equals("ping")) {			//comprobación de que la cadena recibida es ping
			testText = "pingok";
			
			
		}else if (messageFromClient.startsWith("ping&")) { 			// ampliacion ping
			String ID_protocolo = messageFromClient.substring(5); 	//extraemos la id del mensaje
			
			if (ID_protocolo.equals(NanoFiles.PROTOCOL_ID)) {
				testText = "welcome";
			} else {
				testText = "denied";
			}
			
			System.err.print("Error pkt is not ping");
			
		}
		byte[] respData = testText.getBytes();
		// Obtenemos el array de bytes en que se codifica este string (en UDP server hay un ejemplo al devolver los datos al servidor)
		
		DatagramPacket packetToClient = new DatagramPacket(respData, 0 ,respData.length, clientAddr);
		System.out.println("Sending pingo");
	...
	private void sendResponse(DatagramPacket pkt) throws IOException {
		 ...
		 */
		String messageFromClient = new String(pkt.getData(), 0, pkt.getLength());
		System.out.println("[sendResponse] Received:\n" + messageFromClient);
		...
		case DirMessageOps.OPERATION_PING: {
			 ...
			 */
			String receivedProtocolId = msgReceived.getProtocolId();
			boolean compatible = NanoFiles.PROTOCOL_ID.equals(receivedProtocolId);
			 ...
			 */
			System.out.println("[sendResponse] PING protocol='" + receivedProtocolId
					+ "' -> " + (compatible ? "OK (compatible)" : "DENIED (incompatible)"));
			break;
		}

		case DirMessageOps.OPERATION_DIRFILES: {
			msgToSend = new DirMessage(DirMessageOps.OPERATION_AVAILABLEFILES);
			for (FileInfo f : directoryFiles) {
				msgToSend.addFile(f.fileName, f.fileSize, f.fileHash);
			}
			System.out.println("[sendResponse] GET_FILE_LIST -> " + directoryFiles.length + " files");
			break;
		}

		case DirMessageOps.OPERATION_SERVE: {
			String nick = msgReceived.getNickname();
			int port = msgReceived.getServerPort();
			...
			// Resolver colision
			String assignedNick = nick;
			if (registeredPeers.containsKey(assignedNick)) {
				int suffix = 1;
				while (registeredPeers.containsKey(assignedNick + "_" + suffix)) suffix++;
				assignedNick = assignedNick + "_" + suffix;
			}
			registeredPeers.put(assignedNick, serverAddr);
			msgToSend = new DirMessage(DirMessageOps.OPERATION_SERVE_RESPONSE, assignedNick);
			System.out.println("[sendResponse] REGISTER nick='" + assignedNick + "' addr=" + serverAddr);
			break;
		}

		case DirMessageOps.OPERATION_PEERS: {
			msgToSend = new DirMessage(DirMessageOps.OPERATION_PEERLIST);
			for (java.util.Map.Entry<String, InetSocketAddress> entry : registeredPeers.entrySet()) {
				String addrStr = entry.getValue().getAddress().getHostAddress()
						+ ":" + entry.getValue().getPort();
				msgToSend.addPeer(entry.getKey(), addrStr);
			}
			System.out.println("[sendResponse] GET_PEER_LIST -> " + registeredPeers.size() + " peers");
			break;
		}

		case DirMessageOps.OPERATION_QUIT: {
			String nick = msgReceived.getNickname();
			if (nick != null && registeredPeers.containsKey(nick)) {
				registeredPeers.remove(nick);
				msgToSend = new DirMessage(DirMessageOps.OPERATION_QUIT_RESPONSE);
				System.out.println("[sendResponse] UNREGISTER nick='" + nick + "' -> OK");
			} else {
				msgToSend = new DirMessage(DirMessageOps.OPERATION_QUIT_FAIL); 
				System.err.println("[sendResponse] UNREGISTER nick='" + nick + "' -> FAIL (not found)");
			}
			break;
		}
		...
		InetSocketAddress destAddr = (InetSocketAddress) pkt.getSocketAddress();
		DatagramPacket responsePacket = new DatagramPacket(responseBytes, responseBytes.length, destAddr);
