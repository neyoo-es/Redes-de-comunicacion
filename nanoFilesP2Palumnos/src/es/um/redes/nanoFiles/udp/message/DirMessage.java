...
public class DirMessage {
	...
	// Listas de ficheros con nombres, tamaños y hash 
	...
	// Para mensajes de registro de servidor 
	...
	// Añade un fichero nombre, tamaño y hash
	...
	// Añade un peer (nickname y dirección ip:puerto) a los mensajes
	...
	public static DirMessage fromString(String message) {
		...
		for (String line : lines) {
			if (line.trim().isEmpty()) continue; // fin de mensaje
			int idx = line.indexOf(DELIMITER); // Pos delimitador
			if (idx < 0) continue;
			...
			case FIELDNAME_FILENAME: {
				// Guardamos el nombre del fichero, y añadir cuando llegue su filesize
			...
			case FIELDNAME_FILEHASH: {
				// hash contiene los tres campos del fichero, añadir la entrada
	...
	public String toString() {
		...
		case DirMessageOps.OPERATION_QUIT_RESPONSE:
		case DirMessageOps.OPERATION_QUIT_FAIL:
		...
		case DirMessageOps.OPERATION_SERVE_RESPONSE: {
			// Respuesta de registro ok
			if (nickname != null) sb.append(FIELDNAME_NICKNAME + DELIMITER + nickname + END_LINE);
			break;
		}
		case DirMessageOps.OPERATION_SERVE_FAIL: {
			break;
		}
		case DirMessageOps.OPERATION_AVAILABLEFILES: {
