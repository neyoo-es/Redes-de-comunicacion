...
public class DirectoryConnector {
	...
	private byte[] sendAndReceiveDatagrams(byte[] requestData) {
		...
		try {
			...
			while(receive_try < MAX_NUMBER_OF_ATTEMPTS) {
				...
				try {
				//Enviar al servidor - enviamos el paquete
	...
	public boolean testSendAndReceive() {
		...
		if (bytesRespond != null) {
			...
			//si la respuesta es pingok -> exito, si no error
			if (stringRespond.startsWith("pingok")) { // tambien se puede usar .contains pero 
													   // startwith compureba si empieza por ping ok como nos piden 
													   // y es mas seguro
				success = true;
	...
	public boolean pingDirectoryRaw() {
		...
		if (bytesRespond != null) {
			...
			//si la respuesta es welcome -> exito, si no error
	...
	public boolean registerFileServer(int serverPort) {
		...
		DirMessage msgReg = new DirMessage(DirMessageOps.OPERATION_REGISTER_SERVER,
		...
		if (responseData != null) {
			...
			if (DirMessageOps.OPERATION_REGISTER_OK.equals(op)) {
	...
	public FileInfo[] getFileList() {
		...
		// TODO: Ver TODOs en pingDirectory y seguir esquema similar
		DirMessage msgReq = new DirMessage(DirMessageOps.OPERATION_GET_FILE_LIST);
		byte[] requestData = msgReq.toString().getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		if (responseData != null) {
			DirMessage msgResponse = DirMessage.fromString(new String(responseData));
			if (DirMessageOps.OPERATION_FILE_LIST.equals(msgResponse.getOperation())) {
	...
	public Map<String, InetSocketAddress> getPeerList() {
		...
		DirMessage msgReq = new DirMessage(DirMessageOps.OPERATION_GET_PEER_LIST);
		byte[] requestData = msgReq.toString().getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		if (responseData != null) {
			DirMessage msgResponse = DirMessage.fromString(new String(responseData));
			if (DirMessageOps.OPERATION_PEER_LIST.equals(msgResponse.getOperation())) {
	...
	public boolean unregisterFileServer() {
		...
		DirMessage msgUnreg = new DirMessage(DirMessageOps.OPERATION_UNREGISTER_SERVER,
				NanoFiles.peerNickname);
		byte[] requestData = msgUnreg.toString().getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		if (responseData != null) {
			DirMessage msgResponse = DirMessage.fromString(new String(responseData));
			if (DirMessageOps.OPERATION_UNREGISTER_OK.equals(msgResponse.getOperation())) {
				success = true;
			} else {
