...
public class NFController {
	 ...
	 */
	private static final byte CONNECTED  = 1;  // ping OK con el directorio
	private static final byte SERVING    = 2;  // servidor registrado en el directorio
	/*
	...
	public void processCommand() {
		...
		case NFCommands.COM_DOWNLOAD_PEER:
			/*
			 * Punto 4 del boletín SocketsTCP: cuando testModeTCP está activo se ejecuta
			 * el cliente TCP de prueba; cuando no, se realiza la descarga real por peers.
			 */
		...
		switch (currentCommand) {
		    ...
		    case NFCommands.COM_DOWNLOAD_PEER:
		        if (currentState == OFFLINE) {
		            System.err.println("* You must ping the directory first (use 'ping')");
		            commandAllowed = false;
		        }
		        break;
		    case NFCommands.COM_SERVE:
		        // Solo si estamos CONNECTED (no si ya estamos SERVING)
		        if (currentState == OFFLINE) {
		            System.err.println("* You must ping the directory first (use 'ping')");
		            commandAllowed = false;
		        ...
		        break;
		    case NFCommands.COM_NICK:
		        // No se puede cambiar nick mientras se está sirviendo
		        if (currentState == SERVING) {
		            System.err.println("* Cannot change nickname while serving files");
		            commandAllowed = false;
		        }
		        break;
		    case NFCommands.COM_QUIT:
	...
	private void updateCurrentState(boolean success) {
		...
		switch (currentCommand) {
		    case NFCommands.COM_PING:
		        // Ping exitoso: pasamos a CONNECTED (si no estamos ya sirviendo)
		        if (currentState == OFFLINE) {
		            currentState = CONNECTED;
		        }
		        break;
		    case NFCommands.COM_SERVE:
		        // Servidor registrado: pasamos a SERVING
		        currentState = SERVING;
		        break;
		    case NFCommands.COM_QUIT:
		        // Al salir volvemos a OFFLINE
		        currentState = OFFLINE;
		        break;
		    default:
		        // El resto de comandos no cambian el estado
		        break;
		}
