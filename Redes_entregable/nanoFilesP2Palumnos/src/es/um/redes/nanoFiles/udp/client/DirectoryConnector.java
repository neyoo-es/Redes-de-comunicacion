...
public class DirectoryConnector {
	...
	private byte[] sendAndReceiveDatagrams(byte[] requestData) {
		...
		byte responseData[] = new byte[DirMessage.PACKET_MAX_SIZE]; // buffer de recpeción
		byte response[] = null;	
		...
		try {
			...
			while(receive_try < MAX_NUMBER_OF_ATTEMPTS) {
				...
				//Copia de la respuesta, recortando el buffer de ceros
				...
				return response;
					...
					receive_try++;
					System.out.println("run timeout error number of attempts: " + receive_try);
				}
				
			}
		
		} catch (IOException e) {
			System.err.println("DirectoryConnector.sendAndReceiveDatagrams: UDP server connection error: " + e.getMessage());
			System.exit(-1);
	...
	public boolean testSendAndReceive() {
		...
		// llamada al metodo anterior para obtener los datos 
		byte[] bytesRespond = sendAndReceiveDatagrams(requestData);
 
		if (bytesRespond != null) {
			
			String stringRespond = new String(bytesRespond);
			
			if (stringRespond.startsWith("pingok")) { // como debemos comprobar que empieza pingok no usamos .contains si no startswith
				success = true;
	...
	public boolean pingDirectoryRaw() {
		...
		String protocol_message = "ping&" + NanoFiles.PROTOCOL_ID; // mensaje a enviar (String "ping&protocolId")
		byte[] requestData = protocol_message.getBytes();
		byte[] bytesRespond = sendAndReceiveDatagrams(requestData);
		...
		if (bytesRespond != null) {
			...
			if (stringRespond.equals("welcome")) { 
				success = true;
	...
	public boolean pingDirectory() {
		    ...
		    // 1. Construir mensaje  PROTOCOL_ID
		    DirMessage request = new DirMessage(DirMessageOps.OPERATION_PING, NanoFiles.PROTOCOL_ID);
		    ...
		    if (DirMessageOps.OPERATION_WELCOME.equals(op)) {
		        System.out.println("* Directory ping OK: protocol compatible");
		        success = true;
		    } else if (DirMessageOps.OPERATION_DENIED.equals(op)) {
		        System.err.println("* Directory ping DENIED: incompatible protocol ID");
		    } else {
		        System.err.println("* Directory ping: unexpected response op=" + op);
		    }
	...
	public boolean registerFileServer(int serverPort) {
		...
		DirMessage msgReg = new DirMessage(DirMessageOps.OPERATION_SERVE,
				NanoFiles.peerNickname, serverPort);
		byte[] requestData = msgReg.toString().getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		if (responseData != null) {
			DirMessage msgResponse = DirMessage.fromString(new String(responseData));
			String op = msgResponse.getOperation();
			if (DirMessageOps.OPERATION_SERVE_RESPONSE.equals(op)) {
				// El directorio puede haber asignado un nuevo nickname si había colisión
				String assignedNick = msgResponse.getNickname();
				if (assignedNick != null && !assignedNick.equals(NanoFiles.peerNickname)) {
					System.out.println("* Nickname collision: assigned nickname is '" + assignedNick + "'");
					NanoFiles.peerNickname = assignedNick;
				}
				success = true;
			} else {
				System.err.println("* Register failed (op=" + op + ")");
			}
		}
		return success;
	}
	...
	public FileInfo[] getFileList() {
		...
		// TODO: Ver TODOs en pingDirectory y seguir esquema similar
		DirMessage msgReq = new DirMessage(DirMessageOps.OPERATION_DIRFILES);
		byte[] requestData = msgReq.toString().getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		if (responseData != null) {
			DirMessage msgResponse = DirMessage.fromString(new String(responseData));
			if (DirMessageOps.OPERATION_AVAILABLEFILES.equals(msgResponse.getOperation())) {
				java.util.List<String> names = msgResponse.getFilenames();
				java.util.List<Long> sizes = msgResponse.getFilesizes();
				java.util.List<String> hashes = msgResponse.getFilehashes();
				filelist = new FileInfo[names.size()];
				for (int i = 0; i < names.size(); i++) {
					filelist[i] = new FileInfo(hashes.get(i), names.get(i), sizes.get(i), null);
				}
			}
		}
		return filelist;
	...
	public Map<String, InetSocketAddress> getPeerList() {
		...
		DirMessage msgReq = new DirMessage(DirMessageOps.OPERATION_PEERS);
		byte[] requestData = msgReq.toString().getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		if (responseData != null) {
			DirMessage msgResponse = DirMessage.fromString(new String(responseData));
			if (DirMessageOps.OPERATION_PEERLIST.equals(msgResponse.getOperation())) {
				java.util.List<String> nicknames = msgResponse.getPeerNicknames();
				java.util.List<String> addresses = msgResponse.getPeerAddresses();
				for (int i = 0; i < nicknames.size(); i++) {
					String[] parts = addresses.get(i).split(":");
					try {
						InetAddress ip = InetAddress.getByName(parts[0]);
						int port = Integer.parseInt(parts[1]);
						peers.put(nicknames.get(i), new InetSocketAddress(ip, port));
					} catch (Exception e) {
						System.err.println("getPeerList: error parsing address " + addresses.get(i));
					}
				}
			}
		}
	...
	public boolean unregisterFileServer() {
		...
		DirMessage msgUnreg = new DirMessage(DirMessageOps.OPERATION_QUIT,
				NanoFiles.peerNickname);
		byte[] requestData = msgUnreg.toString().getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		if (responseData != null) {
			DirMessage msgResponse = DirMessage.fromString(new String(responseData));
			if (DirMessageOps.OPERATION_QUIT_RESPONSE.equals(msgResponse.getOperation())) {
				success = true;
			} else {
				System.err.println("* Unregister failed (op=" + msgResponse.getOperation() + ")");
			}
		}
		return success;
	}
